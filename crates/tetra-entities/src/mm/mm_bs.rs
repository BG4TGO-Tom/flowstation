...
impl MmBs {
    ...
    fn rx_u_location_update_demand(&mut self, queue: &mut MessageQueue, mut message: SapMsg) {
        ...
        if !is_new {
            ...
            // Sepura terminals send RoamingLocationUpdating after every PTT release.
            // If registered less than 60s ago, treat as soft re-attach to avoid the
            // deregister→register window where PTT gets "no listeners" and radio disconnects.
            let recently_registered = !swmi_commanded_update
                && self.client_mgr
